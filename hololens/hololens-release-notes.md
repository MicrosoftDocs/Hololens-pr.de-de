...
Dieses neueste Release ist ein monatliches Update auf Version 21H1. Dieses Mal werden jedoch neue Features eingeschlossen. Aus diesem Grund bleibt die Hauptbuildnummer unverändert, und Windows Update gibt ein monatliches Release auf Version 21H1 (Build 20348) an. HoloLens 2 Einstellungen werden weiterhin 21H1 angezeigt, obwohl wir auf dieses Release als 21H2 verweisen. Um sicherzustellen, dass Sie 21H2 erhalten haben, überprüfen Sie, ob die Versionsnummer 20348.1432 oder höher lautet. Sie können ihre Buildnummer auf dem Bildschirm Einstellungen > About anzeigen, um zu bestätigen, dass Sie über den neuesten verfügbaren Build 20348.1432 und neuer informiert sind.

Um auf das neueste Release zu aktualisieren, öffnen Sie die Einstellungen-App, wechseln Sie zu & Sicherheit aktualisieren, und tippen Sie auf Nach Updates suchen. Weitere Informationen zum Verwalten HoloLens Updates finden Sie unter [Verwalten HoloLens Updates.](hololens-updates.md)

| Funktion                 | BESCHREIBUNG                | Benutzer oder Szenario |
|-------------------------|----------------------------|--------------|
| [Modus für bewegliche Plattformen](#moving-platform-mode) | Führt die Betaversion des Moving Platform Mode ein, die bei der Konfiguration die Verwendung von HoloLens 2 bei großen 1500-160-Euro ermöglicht, die eine geringe dynamische Bewegung aufweisen. | All |
...
| [Verbesserungen bei der Protokollsammlung mit geringem Speicher](#low-storage-log-collection-improvements) | Verbesserungen an Protokollsammlungsszenarien in Situationen mit wenig Speicher. | Problembehandlung |
...
| [Verwenden von WDAC- und LOB-Apps](#use-wdac-and-lob-apps) | Ermöglicht ES IT-Administratoren, ihre eigenen Apps zu verwenden und weiterhin WDAC zu verwenden, um andere Apps zu blockieren. | IT-Administratoren |
| [Fehlerbehebungen und Verbesserungen](#fixes-and-improvements) | Fehlerbehebungen und Verbesserungen für HoloLens. | All |
...
✔️ Wenn Sie WDAC verwenden möchten, um den Start von Apps oder Prozessen zu blockieren, aber auch Ihre eigene Reihe von Sperr-Apps verwenden möchten, können Sie jetzt [lob in Ihrer WDAC-Richtlinie zulassen.](#use-wdac-and-lob-apps)

### <a name="moving-platform-mode"></a>Modus für bewegliche Plattformen

Ab [Windows Holographic, Version 21H2,](hololens-release-notes.md#windows-holographic-version-21h2) haben wir Betaunterstützung für die Nachverfolgung auf Plattformen mit geringer dynamischer Bewegung auf HoloLens 2 hinzugefügt. Nachdem Sie den Build installiert und den Moving Platform Mode aktiviert haben, können Sie Ihre HoloLens 2 in umgebungen verwenden, auf die zuvor nicht zugegriffen werden kann, z. B. in großen Booten und großen Booten. Derzeit ist das Feature nur auf die Unterstützung dieser spezifischen beweglichen Plattformen ausgerichtet. Zwar hindert Sie nichts daran, die Verwendung des Features in anderen Umgebungen zu versuchen, der Schwerpunkt bei diesem Feature liegt aber zuerst bei der hinzugefügten Unterstützung für diese Umgebungen.
...
#### <a name="overview-to-try-out-moving-platform-mode"></a>Übersicht zum Testen des Moving Platform Mode
...
In [holografischen Windows Version 21H2](hololens-release-notes.md#windows-holographic-version-21h2)wurden zwei Formen von Feedback zur Offlinediagnose hinzugefügt. Das erste sind Popupbenachrichtigungen, die sowohl beim Start als auch beim Abschluss der Sammlung angezeigt werden. Diese werden angezeigt, wenn der Benutzer angemeldet ist und über Visuals verfügt.
...
Diese Informationen können später auf der Seite [Diagnoseprotokolle angezeigt werden.](hololens-diagnostic-logs.md#offline-diagnostics)
...
1. Drücken Sie die **Tastenkombination** Ein-/Herunterschaltfläche, um [die Offlinediagnose zu erfassen.](hololens-diagnostic-logs.md#offline-diagnostics) 
1. Zeigen Sie die Popupbenachrichtigungen an, und hören Sie Audio-Hinweise dazu, wann Das Erfassen von Protokollen auf Ihrem Gerät gestartet und abgeschlossen ist.

### <a name="low-storage-log-collection-improvements"></a>Verbesserungen bei der Protokollsammlung mit geringem Speicher

In Szenarien, in denen auf einem Gerät anscheinend wenig Speicherplatz zur Verfügung stehen soll, wenn Diagnoseprotokolle gesammelt werden, wird ein zusätzlicher Bericht **StorageDiagnostics.zip** erstellt. Der Schwellenwert für wenig Speicher wird automatisch durch die Windows [Speicherwerts bestimmt.](https://support.microsoft.com/office/use-onedrive-and-storage-sense-in-windows-10-to-manage-disk-space-de5faa9a-6108-4be1-87a6-d90688d08a48)
...
> - Die automatische Anmeldung wird nur für MSA- und AAD unterstützt.
...
Die RequirePrivateStoreOnly-Richtlinie wurde für die HoloLens. Mit dieser Richtlinie kann die Microsoft Store-App so konfiguriert werden, dass nur der private Speicher angezeigt wird, der für Ihre Organisation über [Microsoft Store für Unternehmen.](/microsoft-store/microsoft-store-for-business-overview) Beschränken des Zugriffs nur auf die Apps, die Sie zur Verfügung gestellt haben.
...
Sie können jetzt WDAC verwenden, um den Start von Apps oder Prozessen zu blockieren und weiterhin Ihre eigene Linie von Belangsamkeits-Apps zu verwenden. Sie können sie jetzt in Ihrer WDAC-Richtlinie zulassen. Die Verwendung dieser Richtlinie umfasst das Ausführen einer zusätzlichen Codezeile in PowerShell beim Erstellen der WDAC-Richtlinie. [Sehen Sie sich die hier beschriebenen Schritte an.](/mem/intune/configuration/custom-profile-hololens)

#### <a name="overview-to-try-your-own-apps-while-using-wdac-to-block-others"></a>Übersicht über das Testen ihrer eigenen Apps während der Verwendung von WDAC zum Blockieren anderer Apps
...
- Die Gamepadverarbeitung für 2D-Apps wurde in Insider-Builds deaktiviert. Durch das Entfernen können Apps jetzt die Gamepad-APIs direkt verwenden und Zugriff auf die gesamte Gruppe von Steuerelementen haben und können entwickelt werden, um mehr zu erreichen. Entwickler sollten die Gamepad-APIs verwenden, um Gamepad-Eingaben zu nutzen. Hier ist ein Beispiel für [die Gamepad-Klasse (Windows. Gaming.Input) – Windows UWP-Anwendungen](/uwp/api/windows.gaming.input.gamepad?view=winrt-20348&preserve-view=true).
- Aktivierte eine API für [den zugewiesenen Zugriff,](/uwp/api/windows.system.userprofile.assignedaccesssettings?view=winrt-20348&preserve-view=true) sodass Apps jetzt ermitteln können, ob ein HoloLens im Kioskmodus für den Benutzer ausgeführt wird, der beim HoloLens angemeldet ist.

#### <a name="for-enterprise"></a>Für Enterprise

- Beheben von Problemen beim Melden von Kompatibilitätseigenschaften von HoloLens Geräten Möglicherweise ist ein Neustart erforderlich, damit die richtige Berichterstellung in Insider-Builds ausgelöst wird.  
- Die in der Box enthaltene Version von Remote Assist, die auf neuen Flashs installiert ist, wurde aktualisiert.
- Ein Problem wurde behoben, bei dem oobe nach der ersten Benutzeranmeldung in Szenarien beendet wurde, in denen AAD gruppenbasierte Kioskkonfigurationen verwendet wurden.
...
- Ein Problem wurde behoben, bei dem Xbox Controller und andere Bluetooth LE-Peripheriegeräte nach dem Neustart des Geräts erneut gekoppelt werden mussten, um eine Verbindung herzustellen.
- Ein Problem mit dem Videoencoder wurde behoben, das zu einem kurzen Einfrieren ausgehender Videos während eines Remote Assist Anrufs führen konnte. Wi-Fi Treiber- und Firmwareänderungen, um Wi-Fi Sicherheitsrisiken durch "Fragmente und Forge" zu beheben.
- Wi-Fi Treiber- und Firmwareänderungen, um Wi-Fi Sicherheitsrisiken durch "Fragmente und Forge" zu beheben.
- Wenn Sie den Moving Platform Mode (MPM) verwenden, wird "Down" geschätzt, indem der Mittelwert der Schwerkraft über einen kurzen Zeitraum ermittelt wird. Dieser Wert ersetzt die echte Schwerkraft, wenn sie sich im Moving Platform Mode befindet.
- Regelmäßiges Wackeln in Hologrammen im 3DoF-Modus oder beim Verlust der Nachverfolgung wurde behoben.
- Behebt ein Problem, das sich auf Updates des Release 21H1/21H2 von älteren Versionen auswirkt.

## <a name="windows-holographic-version-20h2---october-2021-update"></a>Windows Holographic, Version 20H2 – Oktober 2021 Update

- Build 19041.1168

Verbesserungen und Fehlerbehebungen im Update:

- Dieses monatliche Qualitätsupdate enthält keine wichtigen Änderungen. Wir empfehlen Ihnen, unseren neuesten Build, Windows Holographic, Version 21H2, auszuprobieren.
...
- Es wurde ein Problem behoben, bei dem Wi-Fi Proxy nicht gespeichert werden konnte, wenn Wi-Fi Eigenschaften-Benutzeroberfläche von **Einstellungen > Network & Internet > Status > Eigenschaften** gestartet wurde.
...
- Hochladen hält an, wenn Ihr Gerät in den Ruhefall fällt oder ausgeschaltet ist. Um sicherzustellen, dass Ihre ausstehenden Uploads abgeschlossen sind, verwenden Sie das Gerät aktiv, bis die Einstellungen Seite "OneDrive ist auf dem neuesten Stand" oder Ihre **Power & Standbyeinstellungen** anpassen.
...
- Behebt größere Videobeschädigungen mit farblich kalibrierten Farben.
...
[Neue Einstellungen-App](#new-settings-app) | Die ältere Einstellungen-App wird durch eine aktualisierte Version mit neuen Features und Einstellungen ersetzt. | Endbenutzer |
...
[Automatische Anmeldung von Besucher für Kioske](#visitor-auto-logon-for-kiosks) | Aktiviert die automatische Anmeldung bei Besucherkonten, die für Kioskmodi verwendet werden können. | IT-Administrator |
...
[New SettingsURIs for Page Einstellungen Visibility (Neue EinstellungenURIs für Einstellungen Sichtbarkeit von Seiten)](#new-settings-uris-for-page-settings-visibility) | Mehr als 20 neue SettingsURIs für Einstellungen/PageVisibilityList-Richtlinie. | IT-Administrator |
...
- Auf der seite Einstellungen ist die Statuszeichenfolge, die Ankn., wann das Farbprofil zuletzt geändert wurde, veraltet, bis Sie diese Seite der Einstellungen neu laden.
...
In diesem Windows Build eingeführt, zeigt der Anmeldebildschirm bei Auswahl von **Anderer Benutzer,** der sich rechts neben dem PIN-Eingabefeld befindet, mehrere Benutzer an, die sich zuvor beim Gerät angemeldet haben. Dadurch können Benutzer ihr Benutzerprofil auswählen und sich dann mit ihren Windows Hello Anmeldeinformationen anmelden. Ein neuer Benutzer kann dem Gerät auch auf dieser Seite Andere Benutzer über die Schaltfläche **Konto hinzufügen** hinzugefügt werden.
...
> [!IMPORTANT]
> Wenn Sie ein **USB-Mikrofon anschließen, wird es nicht automatisch als Eingabegerät festgelegt**. Beim Einstecken einer Reihe von USB-C-Geräten werden Benutzer beobachten, dass die Audiodaten des Geräts automatisch an die Mikrofone umgeleitet werden, aber das HoloLens Betriebssystem priorisiert das interne Mikrofonarray über jedem anderen Eingabegerät. **Führen Sie die folgenden Schritte aus, um ein USB-C-Mikrofon zu verwenden.**
...
Für eine AAD Konfiguration können Benutzer ohne diese Änderung ein ähnliches Ergebnis wie heute erreichen. AAD eingebundene Geräte, die für den Kioskmodus konfiguriert sind, können sich mit einer einzigen Schaltfläche vom Anmeldebildschirm aus bei einem Besucherkonto anmelden. Nach der Anmeldung beim Besucherkonto fordert das Gerät erst dann zur erneuten Anmeldung auf, wenn der Besucher explizit über das Startmenü abgemeldet oder das Gerät neu gestartet wurde.
...
Wenn ein Gerät in älteren Builds über eine Kioskkonfiguration verfügt, bei der es sich um eine Kombination aus global zugewiesenem Zugriff und AAD gruppenmitgliedszugewiesenem Zugriff handelt, wird dem Benutzer im Menü Start[nichts angezeigt,](hololens-kiosk.md#issue---no-apps-are-shown-in-start-menu-in-kiosk-mode)wenn die Ermittlung AAD Gruppenmitgliedschaft fehlgeschlagen ist.

Ab diesem Windows Release wird für die Kioskbenutzeroberfläche ein Fallback auf die globale Kioskkonfiguration (falls vorhanden) ausgeführt, falls während AAD Gruppenkioskmodus Fehler vorliegen.
...
| Datenschutz > Ink-& Typisierung der Personalisierung             | `ms-settings:privacy-speechtyping`                 |
...
Sie können apps jetzt installieren, ohne den Entwicklermodus aktivieren oder Geräteportal verwenden zu müssen.  Laden Sie das Appx-Paket einfach (über USB oder Edge) auf Ihr Gerät herunter, und navigieren Sie im Datei-Explorer zum Appx-Paket, um zum Starten der Installation aufgefordert zu werden.  Alternativ können Sie [eine Installation über eine Webseite initiieren.](/windows/msix/app-installer/installing-windows10-apps-web)  Genau wie Apps, die Sie über die Microsoft Store installieren oder mithilfe der BEREITSTELLUNGsfunktion für branchenspezifische Apps von MDM querladen, müssen Apps mit dem [Signierungstool](/windows/win32/appxpkg/how-to-sign-a-package-using-signtool) digital signiert werden, und das zum Signieren verwendete Zertifikat muss vom HoloLens Gerät [als vertrauenswürdig eingestuft werden,](/windows/win32/appxpkg/how-to-sign-a-package-using-signtool#security-considerations) bevor die App bereitgestellt werden kann.
...
Dieses neueste Release ist ein monatliches Update auf Version 2004, aber dieses Mal enthalten wir neue Features. Die Hauptbuildnummer bleibt unverändert, und Windows Update gibt ein monatliches Release auf Version 2004 (Build 19041) an. Sie können ihre Buildnummer auf dem Bildschirm Einstellungen > Informationen anzeigen, um zu bestätigen, dass Sie über den neuesten verfügbaren Build 19041.1128 und mehr informiert sind. Öffnen Sie zum Aktualisieren auf das neueste Release die Einstellungen-App, wechseln Sie zu & Sicherheit aktualisieren, und tippen Sie auf Nach Updates suchen. Weitere Informationen zum Verwalten HoloLens Updates finden Sie unter [Verwalten HoloLens Updates.](hololens-updates.md)
...
Für Benutzer, die Daten zum Anvisieren mit den Augen oder eine sehr genaue Hologrammpositionierung erfordern, empfehlen wir uncalibrierte Benutzer, die Kalibrierung der Blickverfolgung durchzuführen. Sie ist über die Kalibrierungsaufforderung für die Eyetracking-Kalibrierung oder durch Starten der Einstellungen-App über das Startmenü zugänglich, und wählen Sie dann **System > Calibration > Eye Calibration > Run eye calibration**(Kalibrierung der Augen ausführen) aus.
...
Weitere Informationen zum Anwenden von Bereitstellungspaketen während der OOBE finden Sie in der [Dokumentation zur bereitstellung HoloLens.](hololens-provisioning.md#apply-a-provisioning-package-to-hololens-during-setup)
...
HoloLens 2-Geräte unterstützen jetzt den TenantLockdown-CSP ab Windows [Holographic Version 20H2.](hololens-release-notes.md#windows-holographic-version-20h2)
...
| MixedReality\VolumeButtonDisabled                  | Ermöglicht das Deaktivieren von Lautstärkeschaltflächen, sodass durch Drücken des Tastendrucks die Lautstärke nicht geändert wird.               | 1 Ja, 0 Nein (Standard)                                                |
...
| MixedReality\FallbackDiagnostics                   | Steuert das Verhalten, wann Diagnoseprotokolle erfasst werden können.                               | 0 Deaktiviert, 1 Für Gerätebesitzer aktiviert, 2 Für alle aktiviert (Standard) |
...
- Aktivierte Offlinekiosks, die bis zu 60 AAD mit Gruppen verwendet werden können.
...
1. Lassen Sie Azure AD Benutzer 1 anmelden, wenn das Internet verfügbar ist. Sobald die Benutzeranmeldung und Azure AD Gruppenmitgliedschaft erfolgreich bestätigt wurde, wird der Cache erstellt.
...
Ausführliche Informationen zu diesen Updaterichtlinien und deren Verwendung für HoloLens Geräte finden Sie unter [Verwalten HoloLens Updates.](hololens-updates.md)
...
Forscher haben jetzt die Möglichkeit, den Forschungsmodus auf ihren HoloLens Geräten zu aktivieren, um auf all diese externen Datenströme von Rohbildsensoren zuzugreifen. Der Forschungsmodus für HoloLens 2 bietet auch Zugriff auf die Messungen des Beschleunigungsmessers, Gyroskops und Magnetometers. Um die Privatsphäre der Benutzer zu schützen, sind unformatierte Eyetrackingkamerabilder nicht über den Forschungsmodus verfügbar, aber die Blickrichtung ist über vorhandene APIs verfügbar.
...
- Es wurde ein Problem behoben, das dazu führte, dass exklusive HoloLens-Apps bei Verwendung des HoloLens Emulator mit Hardwarebeschleunigung auf bestimmten Geräten auf die Shell zurückstürzten.
- Es wurde ein Problem mit der HTTPS-Weiterleitung für die Windows Geräteportal auf HoloLens 2 behoben.
...
- Ein Problem wurde behoben, das dazu führte, dass HoloLens Apps ihr Pixelformat so geändert haben, dass sie im HoloLens Emulator schwarz gerendert werden.
...
- Ein Problem wurde behoben, das dazu geführt hat, dass HoloLens Apps ihr Pixelformat so geändert haben, dass sie im HoloLens Emulator schwarz gerendert werden.
- Ein Problem mit dem Starten der Fotos-App beim ersten Start wurde behoben, nachdem das Update vom Release 1903 gestartet wurde.

## <a name="windows-holographic-version-2004"></a>Windows Holographic, Version 2004

- Build: 19041.1103

Das wichtige Softwareupdate von Mai 2020 für HoloLens 2, *Windows Holographic, Version 2004* enthält eine Vielzahl von interessanten neuen Funktionen, z. B. Unterstützung für Windows Autopilot, den dunklen App-Modus, USB Ethernet-Unterstützung für 5G/HOTSPOTS-Hotspots und vieles mehr. Um auf das neueste Release zu aktualisieren, öffnen Sie die   Einstellungen-App, wechseln Sie zu **Update & Security**,und wählen Sie die Schaltfläche Nach Updates **suchen**   aus. 
...
|       Anwendungsinstallationsstatus                 |          Überprüfen des Installationsstatus in der Einstellungen App für Apps per Push an HoloLens 2 per MDM               |
...
|       Sprachbefehle                             |          Unterstützung für zusätzliche Sprachbefehle des Systems zum Steuern HoloLens freihändigen Geräts                           |
...
Einige Benutzer teilen ein HoloLens Gerät für andere Benutzer in einer Arbeits- oder Schulumgebung. Daher ist es wichtig, dass Benutzer problemlos lange Benutzernamen und Kennwörter eingeben müssen. Mit Fast Identity Online (FIDO) können sich alle Personen in Ihrer Organisation (Azure AD Mandant) nahtlos bei HoloLens anmelden, ohne einen Benutzernamen oder ein Kennwort einzugeben.
...
Sie können jetzt Sprachbefehle mit jeder App auf dem Gerät verwenden. Weitere Informationen finden Sie unter [Verwenden Ihrer Stimme zum Betreiben HoloLens](hololens-cortana.md). Siehe auch [Unterstützte Sprachen für HoloLens 2](hololens2-language-support.md).  
...
- Die Entwicklerproduktivität wurde verbessert, indem Kunden große Mengen an Text über das Geräteportal einfügen können.
...
- Die Handverfolgung wurde verbessert, sodass der obere Teil dieses Fingers beim Pokeen mit dem Zeigefinger weniger wahrscheinlich unerwartet gelockt wird.
...
- Es wurden Korrekturen für die letzte Phase der Reproduktion (Last Stage Veröffentlichung, LSR) eingeführt. Verbessertes visuelles Rendering von Hologrammen, um stabiler und präziser zu erscheinen, indem die Tiefe genauer abbucht. Dieses Symptom ist nach diesem Update deutlicher, wenn Apps die Tiefe der Hologramme nicht richtig festlegen.
...
- Behoben: Probleme mit holografischen Anwendungen, die beim Start in einem angehaltenen Zustand hängen bleiben, bis Startmenü geöffnet und dann geschlossen wurden.
